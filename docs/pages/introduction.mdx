...
Receive `verification.completed` events at your own HTTPS endpoint when verification requests are processed. Configure the URL and optional auth token in the dashboard **API Keys** page.
...
- Set up [Webhooks](/webhooks) to receive `verification.completed` events
